...
% MW: Changes to support both a ClusterHost and a MirrorHost
if ~cluster.HasSharedFilesystem
    ...
    isBeingMirrored = mirrorConnection.isJobUsingConnection(job.ID);
    ...
    if ~isBeingMirrored && ~isJobFinished
        ...
        try
            mirrorConnection.resumeMirrorForJob(job);
        catch err
    ...
    elseif isJobFinished
        ...
        try
            mirrorConnection.doLastMirrorForJob(job);
