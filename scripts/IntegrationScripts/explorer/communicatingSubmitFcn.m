...
% MW: mirrorConnection will return an empty string if cluster has a shared
% filesystem, as it's not needed.
...
% MW: Where the workers store job output accessible through the MirrorHost
if cluster.HasSharedFilesystem
    storageLocation = environmentProperties.StorageLocation;
else
    storageLocation = mirrorConnection.JobStorageLocation;
...
% MW: The job directory as accessed by workers on the cluster, accessible through the MirrorHost
if cluster.HasSharedFilesystem
    jobDirectoryOnCluster = cluster.getJobFolderOnCluster(job);
else
    jobDirectoryOnCluster = mirrorConnection.getRemoteJobLocation(job.ID, clusterOS);
end
...
if ~cluster.HasSharedFilesystem
    ...
    mirrorConnection.startMirrorForJob(job);
end
...
if cmdFailed
    if ~cluster.HasSharedFilesystem
        ...
        % MW: Modified for a mirrorConnection in the few lines below 
        [remoteConnection, mirrorConnection] = getRemoteConnection(cluster);
        ...
        if mirrorConnection.isJobUsingConnection(job.ID)
            ...
            try
                mirrorConnection.stopMirrorForJob(job);
            catch err
...
% MW: Store appropriate remoteConnection and mirrorConnection
...
if ~cluster.HasSharedFilesystem
    % Store the mirror host
    jobData.MirrorHost = mirrorConnection.Hostname;
    % Store the remote job storage location
    jobData.RemoteJobStorageLocation = mirrorConnection.JobStorageLocation;
